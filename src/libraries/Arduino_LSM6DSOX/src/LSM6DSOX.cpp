...
int LSM6DSOXClass::begin()
{
  ...
  // Set accelerometer ODR = 3.3kHz, 2g full scale 
  writeRegister(LSM6DSOX_CTRL1_XL, 0x92);

  //set the gyroscope control register to work at 104 Hz, 2000 dps and in bypass mode
  // writeRegister(LSM6DSOX_CTRL2_G, 0x4C);

  // Set gyroscope ODR = 3.3kHz, 250 dps full scale
  writeRegister(LSM6DSOX_CTRL2_G, 0x90);

  // set gyroscope power mode to high performance and bandwidth to 16 MHz
  writeRegister(LSM6DSOX_CTRL7_G, 0x00);

  // Set the ODR config register to BW = ODR/4
...
int LSM6DSOXClass::readSensors(PacketBuffer& packet){
  ...
  if (!readRegisters(LSM6DSOX_OUTX_L_G, (uint8_t*)data, sizeof(data))) {
    packet.packet.values[0] = NAN;
    packet.packet.values[1] = NAN;
    packet.packet.values[2] = NAN;
    packet.packet.values[3] = NAN;
    packet.packet.values[4] = NAN;
    packet.packet.values[5] = NAN;
